...
from .enrich_openapi_spec import enrich_spec
from .oas_to_postman import convert_to_postman
from .postman_collection_to_openapi import convert_to_openapi
...
async def to_oas(file: UploadFile = File(...)):
...
async def from_oas(file: UploadFile = File(...)):
    with tempfile.TemporaryDirectory() as temp_dir:
        tmp = tempfile.NamedTemporaryFile(delete=False)
        try:
            tmp.write(file.file.read())
